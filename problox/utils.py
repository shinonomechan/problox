...
def get_latest_client_path():
    ...
    paths = [
        os.path.join(os.getenv("LOCALAPPDATA"), f"Roblox\\Versions\\{version}"),
        os.path.join(os.getenv("SYSTEMDRIVE"), "\\", f"Program Files (x86)\\Roblox\\Versions\\{version}"),
        os.path.join(os.getenv("SYSTEMDRIVE"), "\\", f"Program Files\\Roblox\\Versions\\{version}")
    ]
